...
function normalizeTokenName(value: string | null) {
  const tokenName = value?.trim();
  return (tokenName || DEFAULT_TOKEN_NAME).toLowerCase().replace(/\\s+/g, "-");
}

export async function GET(request: Request) {
  const { searchParams } = new URL(request.url);
  const tokenName = normalizeTokenName(searchParams.get("tokenName"));
  const vsCurrency = searchParams.get("vsCurrency") || DEFAULT_VS_CURRENCY;
  const apiKey = process.env.COINGECKO_API_KEY;

  if (!apiKey) {
    return Response.json({ error: "Missing COINGECKO_API_KEY." }, { status: 500 });
  }
  ...
  marketUrl.searchParams.set("ids", tokenName);
  marketUrl.searchParams.set("price_change_percentage", "24h");

  const chartUrl = new URL(\`https://api.coingecko.com/api/v3/coins/\${tokenName}/market_chart\`);
  ...
  if (!marketResponse.ok || !chartResponse.ok) {
    return Response.json({ error: "Price provider request failed." }, { status: 502 });
  }
  ...
  if (!market || !prices.length || typeof market.current_price !== "number") {
    return Response.json({ error: "Incomplete price data." }, { status: 502 });
  }
